...
public class LogitechSteeringWheel : MonoBehaviour
{
    ...
    [Header("Force Feedback")] private float vibrationTimer;
    ...
    private void Start()
    {
        ...
        buttonStatus = "";
        forcesLabel =
            "Press the following keys to activate forces and effects on the steering wheel / gaming controller \n";
    ...
    private void Update()
    {
        if (SettingsController.DeviceController ==
            "steeringWheel") //Check for what controller the user wants to use. For now hardcoded in SettingsController.cs
        {
            if (LogitechGSDK.LogiUpdate() && LogitechGSDK.LogiIsConnected(0))
            {
                ...
                if (LogitechGSDK.LogiButtonTriggered(0, 4)) StartCoroutine(vehicleController.ChangeGear(1));
                if (LogitechGSDK.LogiButtonTriggered(0, 5)) StartCoroutine(vehicleController.ChangeGear(-1));
                if (LogitechGSDK.LogiButtonTriggered(0, 7)) vehicleController.ToggleDRS();
                if (LogitechGSDK.LogiButtonTriggered(0, 10)) cameraController.NextCamera();
                if (LogitechGSDK.LogiButtonTriggered(0, 6)) cameraController.SetReverseCam(true);
                if (LogitechGSDK.LogiButtonReleased(0, 6)) cameraController.SetReverseCam(false);
                if (LogitechGSDK.LogiButtonReleased(0, 19)) vehicleController.NextERSMode();
                if (LogitechGSDK.LogiButtonReleased(0, 20)) vehicleController.PreviousERSMode();

                if (LogitechGSDK.LogiButtonTriggered(0, 23)) SceneManager.LoadScene(0);
                ...
                var actualProperties = new LogitechGSDK.LogiControllerPropertiesData();
                ...
                var shifterTipe = LogitechGSDK.LogiGetShifterMode(0);
                var shifterString = "";
                ...
                if (Input.GetKeyUp(KeyCode.LeftArrow)) LogitechGSDK.LogiPlaySideCollisionForce(0, 100);
                if (Input.GetKey(KeyCode.RightArrow)) LogitechGSDK.LogiPlaySideCollisionForce(0, -100);

                //Front Collision Force -> up arrow
                if (Input.GetKeyUp(KeyCode.UpArrow)) LogitechGSDK.LogiPlayFrontalCollisionForce(0, 100);

                //Dirt Road Effect-> I
                if (Input.GetKeyUp(KeyCode.I)) ToggleDirtRoadEffect();

                //Bumpy Road Effect-> B
                if (Input.GetKeyUp(KeyCode.B)) ToggleBumpyRoadEffect();

                //Slippery Road Effect-> L
                if (Input.GetKeyUp(KeyCode.L)) ToggleSlipperyRoadEffect();

                //Surface Effect-> U
                if (Input.GetKeyUp(KeyCode.U)) ToggleSurfaceEffect();

                //Car Airborne -> A
                if (Input.GetKeyUp(KeyCode.A)) ToggleAirborneEffect();

                //Soft Stop Force -> O
                if (Input.GetKeyUp(KeyCode.O)) ToggleSoftStopEffect();
                ...
                //Play leds -> P
                if (Input.GetKeyUp(KeyCode.P)) LogitechGSDK.LogiPlayLeds(0, 20, 20, 20);

                for (var i = 0; i < 9; i++) activeForces += activeForceAndEffect[i];

                if (changeLights >= 5)
                {
                    LogitechGSDK.LogiPlayLeds(0, vehicleController.currentEngineRPM, vehicleController.firstLightOn,
                        vehicleController.redLine);
                    changeLights = 0;
                }
                else
                {
    ...
    private void ApplyForceFeedback()
    {
        ...
        if (terrainInfo2 != null)
            if (terrainInfo2.vibration)
            {
                ...
                if (terrainInfo2.vibrationFrequency > vibrationFrequency)
                    vibrationFrequency = terrainInfo2.vibrationFrequency;
                if (terrainInfo2.vibrationIntensity > vibrationIntensity)
                    vibrationIntensity = terrainInfo2.vibrationIntensity;
            }

        if (vibration)
        {
            var intensity =
                Mathf.Clamp((int)(vehicleController.GetSpeed() * vibrationIntensity), 0,
                    40); // Scale intensity with speed
            var frequency =
                Mathf.Clamp(vehicleController.GetSpeed() / 5f * vibrationFrequency, 1f,
                    50f); // Scale frequency with speed
        ...
        var slipForce = CalculateSlipForce();
        var centeringForce = centeringForceMultiplier * (vehicleController.GetSpeed() / vehicleController.maxSpeed) *
            2.5f / Math.Max(slipForce / 3, 1);
    ...
    private void SimulateVibration(int intensity, float frequency)
    {
        ...
        var interval = 1f / (frequency * 2f); // Half-period for toggling force
        ...
        if (vibrationTimer >= interval)
        {
            ...
            var forceMagnitude = vibrationState ? intensity : -intensity;
    ...
    {
        WheelHit hit;
        if (wheel.isGrounded && wheel.GetGroundHit(out hit)) return Mathf.Abs(hit.sidewaysSlip);
        return 0f;
    }
